...
interface SeccionAnalisisPlanProps {
  onDataChange: (data: any) => void;
  initialData?: any;
}

const SeccionAnalisisPlan: React.FC<SeccionAnalisisPlanProps> = ({ 
  onDataChange, 
  initialData = {} 
}) => {
  ...
  return (
    <div className={styles.dataSection}>
      ...
      <p className="text-gray-600 mb-4 text-sm">
        Complete el análisis médico y el plan de tratamiento para el paciente.
      </p>
      ...
      <div className="space-y-4">
        ...
        <div>
          <label className={styles.formLabel}>
            Análisis y Plan <span className="text-red-500">*</span>
          </label>
          <textarea
            {...register('analisisyplan', { 
              required: 'Análisis y Plan es requerido',
              maxLength: { value: 2000, message: 'Máximo 2000 caracteres' }
            })}
            rows={8}
            maxLength={2000}
            placeholder="Describa el análisis médico y el plan de tratamiento detallado"
            className={styles.formInput}
          />
          {errors.analisisyplan && (
            <p className={styles.errorMessage}>{getErrorMessage(errors.analisisyplan)}</p>
          )}
          <p className="text-xs text-gray-500 mt-1">
            Incluya aquí el análisis médico completo, impresión diagnóstica, plan de tratamiento, 
            medicamentos prescritos, seguimiento requerido y cualquier recomendación específica.
          </p>
        </div>
      </div>
    </div>
...
export default SeccionAnalisisPlan;
